  ...
  try {
    ...
    if (this.isModified('status') && this._original?.status === 'occupied') {
      ...
      if (this.currentGuest) {
        return next(new Error(`Cannot change status of Room ${this.number} while guest is checked in. Please check out the guest first.`));
      }
    }
    
    // If setting status to occupied, ensure no other guest is in this room
    if (this.isModified('status') && this.status === 'occupied') {
      ...
      if (existingGuest && existingGuest._id.toString() !== this.currentGuest?.toString()) {
        return next(new Error(`Room ${this.number} is already occupied by another guest.`));
      }
    }
...
roomSchema.statics.updateRoomStatus = async function(roomId, newStatus, reason = '') {
  try {
    ...
    if (room.status === 'occupied' && newStatus !== 'cleaning') {
      throw new Error(`Cannot change status of occupied room ${room.number} to ${newStatus}`);
    }
...
roomSchema.statics.cleanupCleaningRooms = async function() {
  try {
    ...
    if (roomsToClean.length > 0) {
      ...
      console.log(`✅ Cleaned up ${roomsToClean.length} rooms from cleaning status`);
    }
